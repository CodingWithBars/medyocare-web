...
export default async function StudentPage({ params }) {
  ...
  const db = client.db("medcare_db"); // replace with your MongoDB DB name

  // Fetch patient
  const patient = await db
    .collection("patients")
    .findOne({ patient_id: studentId });

  if (!patient) {
    return (
      <div className="p-6 max-w-xl mx-auto text-center">
        <h1 className="text-2xl font-bold text-red-600">Student Not Found</h1>
        <p className="text-gray-600 mt-2">No medical records for ID: {studentId}</p>
      </div>
    );
  }
